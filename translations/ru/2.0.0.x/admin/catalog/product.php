<?php
// Heading
$_['heading_title']          = 'Товары';

// Text
$_['text_success']           = 'Успех: Вы изменили товар!';
$_['text_list']              = 'Список товаров';
$_['text_add']               = 'Добавить товар';
$_['text_edit']              = 'Изменить товар';
$_['text_plus']              = '+';
$_['text_minus']             = '-';
$_['text_default']           = 'По умолчанию';
$_['text_option']            = 'Настройки';
$_['text_option_value']      = 'Значение параметра';
$_['text_percent']           = 'Проценты';
$_['text_amount']            = 'Фиксированная цена';

// Column
$_['column_name']            = 'Название товара';
$_['column_model']           = 'Артикул';
$_['column_image']           = 'Изображение';
$_['column_price']           = 'Цена';
$_['column_quantity']        = 'Количество';
$_['column_status']          = 'Статус';
$_['column_action']          = 'Действие';

// Entry
$_['entry_name']             = 'Название товара';
$_['entry_description']      = 'Описание';
$_['entry_meta_title'] 	     = 'Заголовки Мета-тегов';
$_['entry_meta_keyword'] 	 = 'Ключевые слова Мета-тегов';
$_['entry_meta_description'] = 'Описание Мета-тегов';
$_['entry_keyword']          = 'SEO ключевые слова';
$_['entry_model']            = 'Артикул';
$_['entry_sku']              = 'SKU';
$_['entry_upc']              = 'UPC';
$_['entry_ean']              = 'EAN';
$_['entry_jan']              = 'JAN';
$_['entry_isbn']             = 'ISBN';
$_['entry_mpn']              = 'MPN';
$_['entry_location']         = 'Местоположение';
$_['entry_shipping']         = 'Требуется доставка';
$_['entry_manufacturer']     = 'Производитель';
$_['entry_store']            = 'Магазины';
$_['entry_date_available']   = 'Дата поступления';
$_['entry_quantity']         = 'Количество';
$_['entry_minimum']          = 'Минимальное количество';
$_['entry_stock_status']     = 'Статус "Нет в наличии"';
$_['entry_price']            = 'Стоимость';
$_['entry_tax_class']        = 'Класс Налога';
$_['entry_points']           = 'Баллы';
$_['entry_option_points']    = 'Баллы';
$_['entry_subtract']         = 'Вычитать со склада';
$_['entry_weight_class']     = 'Класс веса';
$_['entry_weight']           = 'Вес';
$_['entry_dimension']        = 'Габаритные размеры (Д x Ш x В)';
$_['entry_length_class']     = 'Класс длины';
$_['entry_length']           = 'Длина';
$_['entry_width']            = 'Ширина';
$_['entry_height']           = 'Высота';
$_['entry_image']            = 'Картинка';
$_['entry_customer_group']   = 'Группы клиентов';
$_['entry_date_start']       = 'Дата начала';
$_['entry_date_end']         = 'Дата окончания';
$_['entry_priority']         = 'Приоритет';
$_['entry_attribute']        = 'Атрибут';
$_['entry_attribute_group']  = 'Группы атрибутов';
$_['entry_text']             = 'Текст';
$_['entry_option']           = 'Опция';
$_['entry_option_value']     = 'Значение параметра';
$_['entry_required']         = 'Обязательно';
$_['entry_status']           = 'Статус';
$_['entry_sort_order']       = 'Порядок сортировки';
$_['entry_category']         = 'Категории';
$_['entry_filter']           = 'Фильтры';
$_['entry_download']         = 'Загрузки';
$_['entry_related']          = 'Связанные продукты';
$_['entry_tag']          	 = 'Теги Продуктов';
$_['entry_reward']           = 'Баллы вознаграждения';
$_['entry_layout']           = 'Вид интерфейса';
$_['entry_recurring']        = 'Профиль периодических платежей';

// Help
$_['help_keyword']           = 'Не используйте пробелы, замените пробелы на "-" и убедитесь в уникальности ключевого слова.';
$_['help_sku']               = 'Единица хранения на складе';
$_['help_upc']               = 'Универсальный товарный код';
$_['help_ean']               = 'Европейский Артикул';
$_['help_jan']               = 'Японский Артикул';
$_['help_isbn']              = 'ISBN';
$_['help_mpn']               = 'Номер партии производителя';
$_['help_manufacturer']      = '(Автозаполнение)';
$_['help_minimum']           = 'Минимальная сумма заказа';
$_['help_stock_status']      = 'Статус показа, когда продукта нет в наличии';
$_['help_points']            = 'Количество баллов, необходимых для покупки этого продукта. Если вы не хотите чтобы этот продукт приобретался за баллы оставьте как 0.';
$_['help_category']          = '(Автозаполнение)';
$_['help_filter']            = '(Автозаполнение)';
$_['help_download']          = '(Автозаполнение)';
$_['help_related']           = '(Автозаполнение)';
$_['help_tag']          	 = 'через запятую';

// Error
$_['error_warning']          = 'Внимание: Проверьте форму на наличие ошибок!';
$_['error_permission']       = 'Внимание: Вы не имеете разрешения на изменение продукции!';
$_['error_name']             = 'Название продукта должно быть больше чем 3 и меньше 255 символов!';
$_['error_meta_title']       = 'Мета-тег Title должен быть от 3 до 255 символов!';
$_['error_model']            = 'Модель товара должна быть больше 1 и меньше, чем 64 символов!';
$_['error_keyword']          = 'Ключевые слова SEO  уже используются!';