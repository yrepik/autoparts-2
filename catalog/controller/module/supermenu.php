...
class ControllerModuleSupermenu extends Controller {
	...
	public function index() {
		...
		$data['categ_text'] = $this->language->get('heading_title');
		...
		if ($vaname[$this->config->get('config_language_id')]) {
			$data['viewalltext'] = $vaname[$this->config->get('config_language_id')];
		}
		...
		if ($caname[$this->config->get('config_language_id')]) {
			$data['categ_text'] = $caname[$this->config->get('config_language_id')];
		}
		...
		$data['dropdowntitle'] = ($this->config->get('supermenu_dropdowntitle') ? true : false);
		...
		$increaseid = 0;
		 ...
		 if (in_array($this->config->get('config_store_id'), $stores)) {  /* check if item is activated for active store */
		  ...
		  if ($item['type'] == 'cat') {
		   ...
		   if ($info_info) {
			...
			if ($item['products'] == 'special') {
			 ...
			 if ($this->config->get('supermenu_fproduct')){
			 $productresults = $this->config->get('supermenu_fproduct');		
